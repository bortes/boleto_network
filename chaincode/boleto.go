...
func (s *SmartContract) registrarBoleto(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if err != nil {
		return shim.Error("01. " + err.Error())
	}
	...
	if err1 != nil {
		return shim.Error("02. " + err1.Error())
	}
	...
	if err2 != nil {
		return shim.Error("03. " + err2.Error())
	}
	...
	if err4 != nil {
		return shim.Error("04. " + err4.Error())
	}
	...
	//if err4 != nil {
	//	return shim.Error("05. " + err4.Error())
	//}
	objBoleto.Valor = "100"
...
func (s *SmartContract) pagarBoleto(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	dataVencimento, err := time.Parse("02/01/2006", objBoleto.DataVencimento)

	now := time.Now()
	if dataVencimento.After(now) {
		return shim.Error("boleto vencido")
	}
	...
	bancoSerializado, err := APIstub.GetState(args[1])
